...
class SchNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.lin2 = Linear(hidden_channels // 2, dim_out)
    ...
    def forward(self, g) -> Tensor:
        ...
        Args:
            ...
            batch (torch.Tensor, optional): Batch indices assigning each atom
                ...
                (default: :obj:`None`)
        """
        batch = torch.zeros_like(g.z) if g.batch is None else g.batch

        h = self.embedding(g.z)
        edge_index, edge_weight = self.interaction_graph(g.pos, batch)
        ...
        if self.dipole:
            # Get center of mass.
            mass = self.atomic_mass[g.z].view(-1, 1)
            M = self.sum_aggr(mass, batch, dim=0)
            c = self.sum_aggr(mass * g.pos, batch, dim=0) / M
            h = h * (g.pos - c.index_select(0, batch))

        if not self.dipole and self.mean is not None and self.std is not None:
            h = h * self.std + self.mean

        if not self.dipole and self.atomref is not None:
            h = h + self.atomref(g.z)
