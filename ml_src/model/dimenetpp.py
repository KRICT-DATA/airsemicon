    ...
    def forward(self, g) -> Tensor:
        ...
        Args:
            ...
            batch (torch.Tensor, optional): Batch indices assigning each atom
                ...
                (default: :obj:`None`)
        """
        edge_index = radius_graph(g.pos, r=self.cutoff, batch=g.batch,
                                  max_num_neighbors=self.max_num_neighbors)

        i, j, idx_i, idx_j, idx_k, idx_kj, idx_ji = triplets(
            edge_index, num_nodes=g.z.size(0))

        # Calculate distances.
        dist = (g.pos[i] - g.pos[j]).pow(2).sum(dim=-1).sqrt()

        # Calculate angles.
        pos_i = g.pos[idx_i]
        pos_ji, pos_ki = g.pos[idx_j] - pos_i, g.pos[idx_k] - pos_i
        ...
        # Embedding block.
        x = self.emb(g.z, rbf, i, j)
        P = self.output_blocks[0](x, rbf, i, num_nodes=g.pos.size(0))

        # Interaction blocks.
        for interaction_block, output_block in zip(self.interaction_blocks,
                                                   self.output_blocks[1:]):
            x = interaction_block(x, rbf, sbf, idx_kj, idx_ji)
            P = P + output_block(x, rbf, i, num_nodes=g.pos.size(0))

        if g.batch is None:
            return P.sum(dim=0)
        else:
            return scatter(P, g.batch, dim=0, reduce='sum')
